# cifar100_arch_sensitivity_iclr.py
# Architecture-sensitivity script for CIFAR-100:
# - MAP
...
try:
    ...
    # allow running from the flattened export directory used in the supplementary archive
...
class CFG:
    ...
    num_workers: int = 2
    num_classes: int = 100
    # architecture-sensitivity setting. Valid values: resnet18, wrn16_4, vgg11_bn.
    arch: str = "wrn16_4"
    ood_svhn_n: int = 2000
...
# ------------------------------------------------------------------------------
# Additional CIFAR architectures for architecture-sensitivity checks
# ------------------------------------------------------------------------------
class WideBasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, dropout_rate=0.0):
        ...
        self.shortcut = None if self.equal_in_out else nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, padding=0, bias=False)
...
class WideResNetCIFAR(nn.Module):
    """WideResNet-16-4 for CIFAR-size images.

    This architecture is intentionally distinct from ResNet-18-CIFAR while keeping
    the experiment light enough for a 3-seed appendix benchmark.
    """
    def __init__(self, depth=16, widen_factor=4, num_classes=100, use_dropout=False, p_drop=0.0):
        super().__init__()
        assert (depth - 4) % 6 == 0, 'WRN depth should be 6n+4.'
        n = (depth - 4) // 6
        k = widen_factor
        widths = [16, 16 * k, 32 * k, 64 * k]
        dropout_rate = p_drop if use_dropout else 0.0
        self.conv1 = nn.Conv2d(3, widths[0], kernel_size=3, stride=1, padding=1, bias=False)
        self.block1 = self._make_layer(widths[0], widths[1], n, stride=1, dropout_rate=dropout_rate)
        self.block2 = self._make_layer(widths[1], widths[2], n, stride=2, dropout_rate=dropout_rate)
        self.block3 = self._make_layer(widths[2], widths[3], n, stride=2, dropout_rate=dropout_rate)
    ...
    def _make_layer(self, in_planes, out_planes, num_blocks, stride, dropout_rate):
        ...
        layers = []
        for st in strides:
            layers.append(WideBasicBlock(in_planes, out_planes, st, dropout_rate=dropout_rate))
...
class VGG11BNCIFAR(nn.Module):
    """Compact VGG-11 with BatchNorm for CIFAR-size images."""
...
def make_model(cfg: CFG, use_dropout: bool = False) -> nn.Module:
    arch = cfg.arch.lower()
    p_drop = cfg.mcdo_p if use_dropout else 0.0
    if arch in ('resnet18', 'resnet18_cifar', 'resnet-18-cifar'):
        return ResNetCIFAR(cfg.num_classes, use_dropout=use_dropout, p_drop=p_drop)
    if arch in ('wrn16_4', 'wrn-16-4', 'wide_resnet16_4'):
        return WideResNetCIFAR(depth=16, widen_factor=4, num_classes=cfg.num_classes, use_dropout=use_dropout, p_drop=p_drop)
    if arch in ('vgg11_bn', 'vgg-11-bn'):
        return VGG11BNCIFAR(num_classes=cfg.num_classes, use_dropout=use_dropout, p_drop=p_drop)
    raise ValueError(f'Unknown architecture: {cfg.arch}')


def architecture_name(cfg: CFG) -> str:
    arch = cfg.arch.lower()
    if arch in ('wrn16_4', 'wrn-16-4', 'wide_resnet16_4'):
        return 'WideResNet-16-4-CIFAR'
    if arch in ('vgg11_bn', 'vgg-11-bn'):
        return 'VGG-11-BN-CIFAR'
    return 'ResNet-18-CIFAR'
...
def load_or_train_map(tr_loader_aug, cfg: CFG) -> nn.Module:
    ...
    model = make_model(cfg, use_dropout=False).to(DEVICE)
...
def deep_ensemble(tr_loader_aug, ts_loader, ood_loader, cfg: CFG):
    ...
    for i in range(cfg.ens_M):
        m = make_model(cfg, use_dropout=False).to(DEVICE)
...
def load_or_train_mcdo(tr_loader_aug, cfg: CFG):
    ...
    model = make_model(cfg, use_dropout=True).to(DEVICE)
    if os.path.exists(path):
        model.load_state_dict(torch.load(path, map_location=DEVICE))
...
def main_iclr(cfg: CFG, methods: List[str], results_path: Optional[str] = None):
    ...
    if wants('map'):
        ...
        rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'MAP', cfg.seed, p_map, p_map_ood, targets_ts, cfg, t))
    ...
    if need_laplace:
        ...
        if wants('ela'):
            rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'ELA', cfg.seed, p_ela, p_ela_ood, targets_ts, cfg, t))
        if wants('lla'):
            rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'LLA', cfg.seed, p_lla, p_lla_ood, targets_ts, cfg, t))
    ...
    if wants('deepens') or wants('deep_ensemble') or wants('ensemble'):
        ...
        rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'DeepEns', cfg.seed, p_ens, p_ens_ood, targets_ts, cfg, t,
    ...
    if wants('swag'):
        ...
        rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'SWAG-Diag', cfg.seed, p_swag, p_swag_ood, targets_ts, cfg, t))
    ...
    if wants('mcdo') or wants('mc_dropout'):
        ...
        rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'MC-Dropout', cfg.seed, p_mc, p_mc_ood, targets_ts, cfg, t))
    ...
    if wants('trl'):
        ...
        rows.append(_metrics_row('CIFAR-100', architecture_name(cfg), 'TRL', cfg.seed, p_trl, p_trl_ood, targets_ts, cfg, t,
...
def parse_args():
    ...
    p.add_argument('--arch', type=str, default='wrn16_4', choices=['resnet18', 'wrn16_4', 'vgg11_bn'])
...
def cfg_from_args(args) -> CFG:
    ...
    cfg.seed = args.seed
    cfg.arch = args.arch
    arch_tag = args.arch.lower()
    cfg.ckpt_dir = args.ckpt_dir or f'./checkpoints_c100_{arch_tag}_seed{args.seed}'
    # keep checkpoints/results disjoint from the ResNet-18 runs
    cfg.map_ckpt = f'{arch_tag}_cifar100_map.pth'
    cfg.mcdo_ckpt = f'{arch_tag}_cifar100_mcdo.pth'
    cfg.ens_prefix = f'c100_{arch_tag}_ens'
    cfg.swag_stats = f'c100_{arch_tag}_swag_stats.pth'
    cfg.trl_spine = f'c100_{arch_tag}_trl_stage2_spine.pth' 
