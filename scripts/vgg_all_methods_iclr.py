...
from vgg_bn_cifar import VGGCIFAR  # VGG-11-BN (4a arquitetura)
...
class CFG:
    ...
    num_workers: int = 2
    num_classes: int = 100
    architecture: str = "VGG-11-BN-CIFAR"
    ood_svhn_n: int = 2000
    ...
    # checkpoints
    ckpt_dir: str = "./checkpoints_c100_vgg"
    map_ckpt: str = "vgg11bn_cifar100_map.pth"
    mcdo_ckpt: str = "vgg11bn_cifar100_mcdo.pth"
    ens_prefix: str = "c100_ens"
    swag_stats: str = "c100_swag_stats.pth"
    trl_spine: str = "c100_vgg_trl_stage2_spine.pth"
...
class _ResNetCIFAR_unused(nn.Module):
...
def load_or_train_map(tr_loader_aug, cfg: CFG) -> nn.Module:
    ...
    model = VGGCIFAR(cfg.num_classes, use_dropout=False).to(DEVICE)
...
def deep_ensemble(tr_loader_aug, ts_loader, ood_loader, cfg: CFG):
    ...
    for i in range(cfg.ens_M):
        m = VGGCIFAR(cfg.num_classes, use_dropout=False).to(DEVICE)
...
def load_or_train_mcdo(tr_loader_aug, cfg: CFG):
    ...
    model = VGGCIFAR(cfg.num_classes, use_dropout=True, p_drop=cfg.mcdo_p).to(DEVICE)
    if os.path.exists(path):
        model.load_state_dict(torch.load(path, map_location=DEVICE))
...
def main_iclr(cfg: CFG, methods: List[str], results_path: Optional[str] = None):
    ...
    if wants('map'):
        ...
        rows.append(_metrics_row('CIFAR-100', cfg.architecture, 'MAP', cfg.seed, p_map, p_map_ood, targets_ts, cfg, t))
    ...
    if need_laplace:
        ...
        if wants('ela'):
            rows.append(_metrics_row('CIFAR-100', cfg.architecture, 'ELA', cfg.seed, p_ela, p_ela_ood, targets_ts, cfg, t))
        if wants('lla'):
            rows.append(_metrics_row('CIFAR-100', cfg.architecture, 'LLA', cfg.seed, p_lla, p_lla_ood, targets_ts, cfg, t))
    ...
    if wants('deepens') or wants('deep_ensemble') or wants('ensemble'):
        ...
        rows.append(_metrics_row('CIFAR-100', cfg.architecture, 'DeepEns', cfg.seed, p_ens, p_ens_ood, targets_ts, cfg, t,
    ...
    if wants('mcdo') or wants('mc_dropout'):
        ...
        rows.append(_metrics_row('CIFAR-100', cfg.architecture, 'MC-Dropout', cfg.seed, p_mc, p_mc_ood, targets_ts, cfg, t))
    ...
    if wants('trl'):
        ...
        rows.append(_metrics_row('CIFAR-100', cfg.architecture, 'TRL', cfg.seed, p_trl, p_trl_ood, targets_ts, cfg, t,
...
def parse_args():
    ...
    p.add_argument('--results', type=str, default='results/cifar100_vgg.jsonl')
...
def cfg_from_args(args) -> CFG:
    cfg = CFG()
    cfg.seed = args.seed
    cfg.ckpt_dir = args.ckpt_dir or f'./checkpoints_c100_vgg_seed{args.seed}'
